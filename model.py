...
class Outfit(db.Model):
    ...
    outfit_id = db.Column(db.Interger, primary_key=True, autoincrement=True)
    ...
    collection_id = db.relationship('Collection', secondary='collection_outfits', back_populates='outfits')
...
class Collection(db.Model):
    ...
    collection_id = db.Column(db.Interger, primary_key=True, autoincrement=True)
    collection_name = db.Column(db.String(40), nullable=False)
    public = db.Column(db.Boolean, nullable=False, default=True)
    user_id = db.Column(db.Integer, db.ForeignKey('users.user_id'))
    user_id = db.relationship('User', secondary='collection_outfits', back_populates='collections')
...
class Character(db.Model):
    ...
    character_id = db.Column(db.Interger, primary_key=True, autoincrement=True)
...
class Shop(db.Model):
    ...
    shop_id = db.Column(db.Interger, primary_key=True, autoincrement=True)
...
class Item(db.Model):
    ...
    shop_id = db.Column(db.Ineger, db.ForeignKey('shops.shop_id'))
...
class Show(db.Model):
    ...
    show_id = db.Column(db.Interger, primary_key=True, autoincrement=True)
...
def connect_to_db(app)
    app.config["SQLALCHEMY_DATABASE_URI"] = f"postgresql:///cosplay_planner"
